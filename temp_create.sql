CREATE SCHEMA conalaun_testing;
CREATE TABLE `conalaun_testing`.`concepts` (
  `tid` INT NOT NULL AUTO_INCREMENT,  
  `description` VARCHAR(255) NOT NULL,
 PRIMARY KEY (`tid`));
CREATE TABLE `conalaun_testing`.`concept_symbols` (
  `tidref` INT NOT NULL,
  `idx` TINYINT NULL,
  `type` TINYINT NULL,
  `name` VARCHAR(160) NULL);
CREATE TABLE `conalaun_testing`.`rr_catalogue` (
  `cidref` INT NOT NULL,
  `id` TINYINT NOT NULL,
  `name` VARCHAR(160) NULL);
CREATE TABLE `conalaun_testing`.`rr_relations` (
  `cidref` INT NOT NULL,
  `rr_catalogue_id` TINYINT NOT NULL,
  `class_text_lines_type` CHAR(2) NOT NULL,
  `class_text_lines_idx` TINYINT NULL,
  `vn` TINYINT NOT NULL);
CREATE TABLE `conalaun_testing`.`class` (
  ...
  PRIMARY KEY (`cid`));
CREATE TABLE `conalaun_testing`.`class_text_lines` (
  ...
  PRIMARY KEY (`id`));
CREATE TABLE `conalaun_testing`.`class_select_statements` (
  `cidref` INT NOT NULL,
  `idx` TINYINT NOT NULL,
  `sql_select_statement` VARCHAR(255) NULL);
CREATE TABLE `conalaun_testing`.`class_extra_conditions` (
  `cidref` INT NOT NULL,
  `type` CHAR NOT NULL,
  `idx` TINYINT NOT NULL,
  `vn` TINYINT NOT NULL,
  ...
  `is_cons` CHAR NULL);
CREATE TABLE `conalaun_testing`.`rr_group_member_conditions` (
  `cidref` INT NOT NULL,
  `rr_catalogue_id` TINYINT NOT NULL,
  `group_name` VARCHAR(60) NOT NULL,
  `exist` CHAR(1) NOT NULL);
CREATE TABLE `conalaun_testing`.`sr_group_member_conditions` (
  `cidref` INT NOT NULL,
  `sr_name` VARCHAR(40) NOT NULL,
  `group_name` VARCHAR(60) NOT NULL,
  `exist` CHAR(1) NOT NULL);
CREATE TABLE `conalaun_testing`.`smol_variables` (
...
CREATE TABLE `conalaun_testing`.`syn_sr_to_sr` (
   `cidref` INT NOT NULL,
