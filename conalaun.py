...
def variable_fetch(varname):
    if varname == "_CART_INPUT_LINE_NUMBER": return _CART_INPUT_LINE_NUMBER
    if varname == "_CART_FILEDB": return _CART_FILEDB
...
term_manager.variable_fetch = variable_fetch
term_manager.dump_table = cart_dump_table.dumptable
term_manager._CART_INPUT_LINE_NUMBER = _CART_INPUT_LINE_NUMBER
term_manager._CART_FILEDB = _CART_FILEDB
...
try:
    term_manager.update_fast_string_scan_cache(myconn, logging, CON_LIST_FILE, GROUP_DIR_NAME, ALT_TEXT_DIR, True)
except Exception:     
    logging.error("There was an exception thrown in term_manager.update_fast_string_scan_cache() - program terminating.")    
    exit(0)
...
while True:    
    ...
    if TESTING:
        _CART_INPUT_LINE_NUMBER += 1
        #CART_INCLUDE_v1.000_remove_terms.py
    ...
    #CART_INCLUDE_v1.000_testa.py
    #CART_INCLUDE_v1.000_testb.py
    #CART_INCLUDE_v1.000_report_fssc_2.py
